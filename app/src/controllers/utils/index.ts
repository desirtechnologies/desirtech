...
const { defineUtility } = blackprint()

export const notion = defineUtility({
    methods: {
        getProperties: (data) => {
        ...
        multi_select: (data) => {
            return (
                data?.multi_select?.map((item) => (item?.name ? item?.name : null)) ?? [
                    "[Network]: MULTI_SELECT_NOT_FOUND",
                ]
            );
        },
        icon: (data) => {
...
export const collections = defineUtility({
    methods: {
        shuffle: (arr) => {
